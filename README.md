...
├── ccc/                  그냥 파일               ->  /ccc/7/12/99.png
└── korea/                (묶음 폴더, 레이어 아님)
    ├── seoul/            RocksDB                ->  /korea/seoul/12/345/4455.jpg
    └── busan/            RocksDB                ->  /korea/busan/4/222/5555.terrain
```

**레이어 목록에 담는 것은 RocksDB 뿐이다.** 파일은 목록 없이 URL 그대로 내보낸다(일반 웹 서버처럼).
자세한 규칙은 [폴더 여러 단으로 묶기](#폴더-여러-단으로-묶기) 참고.
...
RocksDB 레이어에 걸리지 않은 요청은 전부 파일로 찾는다. **타일 규칙이 없다.** URL 경로가 곧 파일 경로다.
목록에 담지 않으므로 폴더를 미리 훑지도 않는다.
...
**RocksDB 내부 파일은 내보내지 않는다.** 등록된 DB 폴더는 카탈로그가 먼저 가져가지만,
복사가 덜 끝났거나 `MaxLayerDepth` 보다 깊어서 등록되지 않은 DB 가 남을 수 있다.
그 SST 를 그대로 내려받게 두면 DB 전체를 복원할 수 있으므로,
`CURRENT` / `LOCK` / `IDENTITY` / `LOG*` / `MANIFEST-*` / `OPTIONS-*` / `*.sst` / `*.blob` 은 404 로 막는다.
...
**레이어 목록에는 RocksDB 만 담는다.** 파일은 담지 않는다.

폴더를 볼 때 판단은 두 줄이면 끝난다.

1. **폴더가 RocksDB 면 그게 레이어다.** 거기서 끝이고 안으로는 절대 들어가지 않는다.
   DB 안에 무엇이 있어도(파일이든 또 다른 DB 든) 서비스되지 않는다.
2. **RocksDB 가 아니면 `MaxLayerDepth` 까지 안으로 들어가 DB 를 찾는다.**
   그 폴더 자체는 레이어가 아니라 이름만 빌려주는 묶음 폴더다.

파일은 이 판단에 끼지 않는다. 요청이 오면 URL 을 그대로 `{Root}` 밑 파일 경로로 쓴다.
그래서 파일 폴더를 미리 훑을 이유가 없다 — 목록도, 깊이 제한도, 가지 규칙도 필요 없다.

```
{Root}/dem/korea/      RocksDB  ->  /dem/korea/{z}/{x}/{y}.{ext}     목록에 있음
{Root}/raster/korea/   RocksDB  ->  /raster/korea/{z}/{x}/{y}.{ext}  목록에 있음
{Root}/test/           RocksDB  ->  /test/{z}/{x}/{y}.{ext}          목록에 있음
{Root}/test_file/...   파일      ->  /test_file/...                   목록에 없음, 그냥 나감
```

메인 페이지와 `/admin/layers` 에는 앞의 셋만 나온다. 파일은 목록이 없으니 나올 것도 없다.

### 폴더 이름을 숫자만으로 짓지 말 것

**이름이 숫자뿐인 폴더는 안으로 들어가지 않는다.** 타일 레벨(z)로 보기 때문이다.

이게 없으면 DB 를 찾는다고 파일 타일 트리를 파고들어, z/x 폴더 수만큼(레벨 14 면 수만 개)
매 훑기마다 디스크를 뒤진다. z/x/y 는 전부 숫자이고 사람이 짓는 묶음 이름(`korea`, `dem`)은
그렇지 않으니, 이 한 줄짜리 검사로 "여기부터는 타일 트리다" 를 공짜로 구분한다.

그래서 `{Root}/2024/seoul/` 처럼 **숫자만으로 된 묶음 폴더는 인식되지 않는다.**
`y2024` 처럼 글자를 하나 섞으면 된다.

### 목록을 얼마나 자주 훑나

| 계기 | 시점 |
|---|---|
| 주기적 훑기 | `RescanSeconds` 마다 (기본 **60 초**) |
| 폴더 감시 | 루트·묶음 폴더에 변화가 오면 1 초 디바운스 후 |
| `/admin/reload` | 즉시 |
| 떼어내기(detach) 만료 | 즉시 |

배경 작업이 1 초마다 조건만 확인한다(디스크는 안 본다).

훑는 비용은 파일을 목록에 담지 않게 되면서 사실상 사라졌다.
루트 밑에 z/x 폴더 3,085 개짜리 파일 타일 트리를 두고 재봐도 **재훑기 한 번이 2.4 ms** 다
(기준선 `/healthz` 는 0.17 ms). 파일 트리 안으로 들어가지 않기 때문이다.

### 훑기가 서비스에 부하가 되나

**안 된다.** 조회 경로는 재훑기 락을 잡지 않는다.
레이어 사전은 `FrozenDictionary` 스냅샷이라 읽는 쪽은 `Volatile.Read` 한 번이고,
갱신은 새 사전을 만들어 참조만 바꿔 끼운다. 둘이 서로 기다릴 일이 없다.

기본값의 **30 배로 조여서**(2 초마다) 부하를 걸고 재봤다. 순서 효과를 상쇄하려고 ABBA 로 교차 측정했다.

| 순서 | 설정 | RPS | p99 |
|---|---|---|---|
| 1 | 훑기 OFF | 215,881 | 0.46 ms |
| 2 | 2 초마다 훑기 | 159,280 | 0.72 ms |
| 3 | 2 초마다 훑기 | 163,028 | 0.59 ms |
| 4 | 훑기 OFF | **161,441** | 0.65 ms |
| 5 | 훑기 OFF | **161,758** | 0.68 ms |

4·5 번(훑기 OFF)이 2·3 번(2 초 훑기)과 같다. 1 번만 높은 것은 첫 실행 이상치다.
즉 **훑기를 켜고 끄는 차이가 측정되지 않는다.** 기본값 60 초라면 더 말할 것도 없다.

### 새 DB 는 배경에서 미리 연다

훑기 자체보다 신경 쓸 것은 **DB 를 여는 비용**이다.
MANIFEST 를 읽고 SST 를 여는 데 DB 가 크면 수십~수백 ms 가 든다.
이걸 첫 요청 스레드에서 하면 그 요청 하나가 그만큼 멈춘다.

그래서 훑기가 새 DB 를 찾으면 **락을 놓은 뒤 배경 작업 스레드에서 미리 열어둔다.**
요청은 항상 이미 열린 핸들을 받는다.

```
새 DB 투입 -> 목록에 뜬 시점의 상태: "open"   (첫 요청 전에 이미 열려 있다)
첫 요청: 3.3 ms  (열기 비용이 아니라 그 연결의 첫 요청 비용)
```

락 안에서 열면 그동안 `detach`/`attach`/`reload` 가 전부 막히므로 반드시 락 밖에서 연다.

### 서버 상태 보기

첫 화면(`/`)에 상태표가 있고, 같은 내용을 `/status` 로 JSON 으로도 받는다.

```
가동 시간       3일 4시간 12분
시작 시각       2026-09-02 09:56:56 +09:00
현재 시각       2026-09-02 13:09:03 +09:00
버전            1.2608.26
타일 루트       /srv/tiles
RocksDB 레이어  4
...
값은 요청이 올 때 그 자리에서 읽는다. **요청마다 갱신하는 카운터는 두지 않았다.**
...
명령을 내릴 때만 목록을 갱신하게 할 수 있다. 설정 두 줄이면 된다.

```json
"TileServer": {
  "RescanSeconds": 0,        // 주기적 훑기 끔
  "WatchFileSystem": false   // 폴더 감시 끔
}
```

이러면 **`/admin/reload` 를 부를 때만** 목록이 갱신된다. 확인한 동작:

```
DB 를 넣고 8 초 대기       -> 404   (자동으로 잡지 않는다)
POST /admin/reload         -> {"reloaded":true,"count":4}
같은 요청 다시             -> 200
```

`/admin/*` 은 토큰을 설정하지 않았으면 루프백에서만 받는다.
운영 서버에서 원격으로 부르려면 `AdminToken` 을 설정하고 헤더를 실어 보낸다.

```bash
curl -X POST http://127.0.0.1:7080/admin/reload
curl -X POST -H "X-Admin-Token: <토큰>" https://tiles.example.com/admin/reload
```

배포 스크립트 마지막 줄에 넣어두면 "DB 복사 -> reload" 가 한 번에 끝난다.
다만 위 실측대로 훑기는 부하가 아니므로, 기본값(60 초)을 그대로 두는 쪽을 권한다.
자동 훑기를 끄면 DB 를 넣고 reload 를 깜빡했을 때 조용히 404 가 난다.
...
> `MaxLayerDepth: 1` 로 두면 최상위 폴더만 DB 로 인식한다. 상한은 4 다.
...
| `MaxLayerDepth` | `3` | RocksDB 를 찾아 들어갈 깊이(1~4). 파일과 무관 |
| `RescanSeconds` | `60` | **RocksDB 목록** 재훑기 주기. `0` 이면 끔 |
