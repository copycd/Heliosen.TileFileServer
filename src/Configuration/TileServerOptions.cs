...
public sealed class TileServerOptions
{
    ...
    ///
    /// **RocksDB 폴더에만 해당한다.** 파일은 목록에 담지 않으므로 깊이 제한이 없다
    /// (요청 때 URL 을 그대로 파일 경로로 쓴다).
    ///
    ///  - 폴더가 RocksDB 면 그게 레이어이고 거기서 끝이다. DB 안으로는 절대 들어가지 않는다.
    ///  - RocksDB 가 아니면 이 깊이까지 안으로 들어가 DB 를 찾는다. 그 폴더 자체는 레이어가 아니다.
    ///  - 이름이 숫자뿐인 폴더는 타일 레벨(z)로 보고 들어가지 않는다.
    ///    그게 없으면 파일 타일 폴더를 훑을 때마다 z/x 폴더 수만큼 디스크를 뒤진다.
    /// </summary>
    public int MaxLayerDepth { get; set; } = 3;

    /// <summary>
    /// 루트를 다시 훑어 **RocksDB 폴더 목록**을 갱신하는 주기(초). 0 이면 끈다.
    ///
    /// 파일은 목록에 담지 않으므로 이 훑기와 무관하다(요청 때 디스크에서 바로 찾는다).
    ///
